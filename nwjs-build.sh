...
mk_windows() {
    ...
    rm ${WORKING_DIR}/${TMP}/${ARR_OS[$i]}/latest-git/nw.exe
...
get_ffmpeg() {
    ...
    filename="${FFMPEG_VERSION}-${ARR_OS[$i]}.zip"
    url="https://github.com/iteufel/nwjs-ffmpeg-prebuilt/releases/download/${FFMPEG_VERSION}/${filename}"
    localfile=${LOCAL_NW_ARCHIVES_PATH}/${filename}
    printf "Building ${TXT_BOLD}${TXT_YELLO}ffmpeg${TXT_RESET} for ${TXT_BOLD}${TXT_YELLO}${ARR_OS[$i]}${TXT_RESET}\n"
    if [[ -f "${localfile}" ]]; then
        NOTE 'NOTE';
        printf "File ${filename} is in the download cache\n- no need to re-download\n"
        cp ${localfile} ${WORKING_DIR}/${TMP};
    else
        wget -O ${localfile} ${url}
    fi
    extractme "zip" "${localfile}" "${WORKING_DIR}/${TMP}/${ARR_OS[$i]}"
    mv "${WORKING_DIR}/${TMP}/${ARR_OS[$i]}/ffmpeg.dll" "${WORKING_DIR}/${TMP}/${ARR_OS[$i]}/latest-git/"

    cd ${WORKING_DIR};

    NOTE "DONE";
    printf "${filename} has been used."
}
